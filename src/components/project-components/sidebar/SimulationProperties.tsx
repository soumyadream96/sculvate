...
import { setMesh } from "state/reducers/generatedMeshSlice";
...
function SimulationProperties({
  ...
  const [name, setName] = useState<string>(simulationProperties.name);
  const [frequencyUnit, setFrequencyUnit] = useState(
    simulationProperties.frequencyUnit
  );
  const [dimensionsUnit, setDimensionsUnit] = useState(
    simulationProperties.dimensionsUnit
  );
  ...
  const [pml_n] = useState<number>(simulationProperties.pml_n);
  const [end_criteria, setEndCriteria] = useState<number>(
    simulationProperties.end_criteria
  );
  const [xMin, setXMin] = useState(simulationProperties.xMin);
  const [xMax, setXMax] = useState(simulationProperties.xMax);
  const [yMin, setYMin] = useState(simulationProperties.yMin);
  const [yMax, setYMax] = useState(simulationProperties.yMax);
  const [zMin, setZMin] = useState(simulationProperties.zMin);
  const [zMax, setZMax] = useState(simulationProperties.zMax);
  const [padding, setPadding] = useState(simulationProperties.padding);
  ...
  const [roth_field, setRotHField] = useState<Array<number | string>>(
    simulationProperties.roth_field
  );
  ...
  const [originalProperties, setOriginalProperties] = useState<{} | undefined>(
    undefined
  );

  useEffect(() => {
    if (!visible && originalProperties !== undefined) return;
    ...
    (async () => {
      try {
        ...
        if (data.Body) {
          ...
          dispatch(updateSimulationProperties(json));
          setOriginalProperties(json);
        }
      } catch (e) {
        console.log(e);
      }
    })();
  }, [visible, currentUsername, projectId, dispatch]);

  useEffect(() => {
    setName(simulationProperties.name);
    setFrequencyUnit(simulationProperties.frequencyUnit);
    setDimensionsUnit(simulationProperties.dimensionsUnit);
    ...
    setXMin(simulationProperties.xMin);
    setXMax(simulationProperties.xMax);
    setYMin(simulationProperties.yMin);
    setYMax(simulationProperties.yMax);
    setZMin(simulationProperties.zMin);
    setZMax(simulationProperties.zMax);
    setPadding(simulationProperties.padding);
    setFarfield(simulationProperties.farfield);
    setEField(simulationProperties.e_field);
    setHField(simulationProperties.h_field);
    setRotHField(simulationProperties.roth_field);
  }, [
    simulationProperties.name,
    simulationProperties.frequencyUnit,
    simulationProperties.dimensionsUnit,
    ...
    simulationProperties.end_criteria,
    simulationProperties.xMin,
    simulationProperties.xMax,
    simulationProperties.yMin,
    simulationProperties.yMax,
    simulationProperties.zMin,
    simulationProperties.zMax,
    simulationProperties.padding,
    simulationProperties.farfield,
    simulationProperties.e_field,
    simulationProperties.h_field,
    simulationProperties.roth_field,
  ...
  const updateProjectName = async (name: any) => {
    try {
      const data = await Storage.get(
      ...
      if (data.Body) {
        ...
        json.frequency_range = `${f_min} - ${f_max} ${frequencyUnit}`;
  ...
  const savePropertiesData = async (propertiesData: any) => {
    ...
    if (JSON.stringify(propertiesData) !== JSON.stringify(originalProperties)) {
      ...
      buttons={
        <>
          <button
            type="button"
            id="element-ok-btn"
            className={`relative w-20 h-9 flex items-center justify-center shadow-sm ml-2 my-auto font-medium text-sm rounded-md focus:outline-none text-white ${
              !isLoading &&
              name &&
              f_min !== undefined &&
              f_min >= 0 &&
          ...
          <button
            type="button"
            className="relative h-9 items-center ml-2 my-auto mt-3 inline-flex w-20 justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 active:bg-gray-100 sm:mt-0"
        ...
        <div className="grid grid-cols-1 gap-y-4 mx-2">
          ...
          <div className="grid grid-cols-3 gap-x-2">
            <div className="ml-2 col-span-1">
              <h3 className="font-medium underline">Simulation properties</h3>
            ...
            <div className="w-20 ml-52">
              <h3 className="font-medium underline">Units</h3>
              <label
          ...
          <div className="ml-2 grid grid-cols-3 gap-x-1">
            <h3 className="font-medium underline">
              Frequency range ({frequencyUnit})
            </h3>
            <h3 className="font-medium"></h3>
            <h3 className="font-medium underline">Excitation type</h3>
            ...
            <div className="col-span-1 ml-1 mt-2">
              <div className="accent-primary-600 mt-1 grid grid-cols-1">
                <label>
                  <input
                    type="radio"
                    name="excitation"
          ...
          <div className="mb-4 grid grid-cols-3" style={{ minHeight: "150px" }}>
            <div className="col-span-2 mr-4">
              <label className="ml-2 font-medium underline">
                Excitation signal
              </label>
            ...
            <div className="col-span-1 ml-1">
              <label htmlFor="endCriteria" className="font-medium underline">
                End criteria
              </label>
              <label
                htmlFor="fMin"
              ...
              <div className="w-20 flex rounded-md shadow-sm sm:max-w-lg ring-1 ring-inset ring-gray-300 disable-drag mt-2">
                <input
                  type="number"
                  name="endCriteria"
                  id="endCriteria"
          ...
          <div className="grid grid-cols-4 gap-x-48 items-start">
            {/* Boundaries */}
            <div className="ml-2 grid grid-cols-2 col-span-2 gap-x-2 gap-y-2">
              <h3 className="font-medium underline col-span-full">
                Boundaries
              </h3>
            ...
            {/* Feild  */}
            <div className="grid grid-cols-1 gap-x-2 gap-y-2 w-64 mr-0 pr-0">
              <h3 className="font-medium col-span-full underline">
