...
const Statistics = (props: StatisticsProps) => {
  ...
  const downloadJson = async (
    ...
    try {
      ...
      const list = result.results || [];
      const jsonPattern = /\.json$/i; // Regular expression for .json
      const csvPattern = /\.csv$/i; // Regular expression for .csv
      let jsonFilename = "";
      const csvFiles = await Promise.all(
        list.map(async (item: { key: string }, index: number) => {
          if (csvPattern.test(item.key)) {
            const fileName = item.key.split("/").pop()?.slice(0, -4) + ".json";
      ...
      const files = await Promise.all(
        list.map(async (item: { key: string }, index: number) => {
          if (jsonPattern.test(item.key)) {
            ...
            let fileName;
            if (item.key.includes("results.json")) {
  ...
  return (
    <>
        ...
        <li>
          <div className="bg-[#F9FAFB] rounded-md p-2 overflow-hidden">
            <div className=" space-y-4">
              <div>
                <div className=" text-base text-gray-700 font-semibold underline underline-offset-4 mb-1">
                  {`|S11| minima (` + frequencyUnit + `)`}
                </div>
      ...
      <div className="mt-5 space-y-3 bg-[#F9FAFB] rounded-md pt-2 px-2 pb-2.5">
        <div className="bg-[#EAECF0] rounded-lg">
          <a className="flex items-center px-2.5 py-0.5 text-sm font-normal text-[#101828] rounded-lg">
            <MyIcon name="download-results" />
            <span className="ml-3.5 py-0.5 font-bold text-base">
              Download&nbsp;results
            </span>
          </a>
        </div>
        <div className="space-y-3">
          ...
          <button
            ...
            onClick={() => downloadJson(projectId, "v1", username)}
          ...
          >
            <span>
              ...
              >
                ...
                />
                <path
                  d="M10.7637 13.999C10.4397 13.999 10.1487 13.93 9.89071 13.792C9.63871 13.654 9.44371 13.459 9.30571 13.207C9.17371 12.961 9.11971 12.67 9.14371 12.334L9.23371 11.029C9.24571 10.837 9.19171 10.684 9.07171 10.57C8.95171 10.456 8.79571 10.399 8.60371 10.399H7.47871V9.139H8.60371C8.79571 9.139 8.95171 9.082 9.07171 8.968C9.19171 8.848 9.24571 8.695 9.23371 8.509L9.14371 7.15C9.12571 6.82 9.18271 6.535 9.31471 6.295C9.44671 6.049 9.63871 5.86 9.89071 5.728C10.1427 5.596 10.4337 5.53 10.7637 5.53H11.5287V6.79H11.0337C10.8657 6.79 10.7307 6.832 10.6287 6.916C10.5267 7 10.4817 7.138 10.4937 7.33L10.5837 8.689C10.6077 9.019 10.4907 9.286 10.2327 9.49C9.98071 9.688 9.63271 9.787 9.18871 9.787V9.724C9.63271 9.724 9.98071 9.829 10.2327 10.039C10.4907 10.243 10.6077 10.513 10.5837 10.849L10.4937 12.199C10.4877 12.361 10.5357 12.493 10.6377 12.595C10.7397 12.691 10.8717 12.739 11.0337 12.739H11.5287V13.999H10.7637ZM13.4748 13.999V12.739H13.9698C14.1318 12.739 14.2638 12.691 14.3658 12.595C14.4738 12.493 14.5218 12.361 14.5098 12.199L14.4198 10.849C14.3958 10.513 14.5128 10.243 14.7708 10.039C15.0288 9.829 15.3768 9.724 15.8148 9.724V9.787C15.3768 9.787 15.0288 9.688 14.7708 9.49C14.5128 9.286 14.3958 9.019 14.4198 8.689L14.5098 7.33C14.5218 7.138 14.4768 7 14.3748 6.916C14.2788 6.832 14.1438 6.79 13.9698 6.79H13.4748V5.53H14.2398C14.5698 5.53 14.8608 5.596 15.1128 5.728C15.3648 5.86 15.5568 6.049 15.6888 6.295C15.8268 6.535 15.8838 6.82 15.8598 7.15L15.7698 8.509C15.7578 8.695 15.8118 8.848 15.9318 8.968C16.0578 9.082 16.2138 9.139 16.3998 9.139H17.5248V10.399H16.3998C16.2138 10.399 16.0578 10.456 15.9318 10.57C15.8118 10.684 15.7578 10.837 15.7698 11.029L15.8598 12.334C15.8838 12.67 15.8268 12.961 15.6888 13.207C15.5568 13.459 15.3648 13.654 15.1128 13.792C14.8608 13.93 14.5698 13.999 14.2398 13.999H13.4748Z"
                  fill="white"
                />
              </svg>
            </span>
