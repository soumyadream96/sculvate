...
function MainScene({ getScene, setObjects, objects }: MainSceneProps) {
      ...
      arrayModel.forEach((model: any) => {
        if (model.status === "Added") {
          ...
          const objectToCreate = {
            ...
            heightExt: model.height,
            diameter:model.diameter,
            topDiameter:model.topDiameter,
            bottomDiameter:model.bottomDiameter,
            tessellation:model.tessellation,
  ...
  function swapArrayItems(arr: any[], index1: number, index2: number): void {
  ...
  const addShape = async (type: string, obj: any) => {
    ...
    if (type === "cube") {
      ...
      console.log(obj)
      if((obj.tessellation!=undefined)&&(obj.scaling.x==1)&&(obj.selectedFaceScaling.x==1)&&(obj.selectedFacePoints[0].x!=obj.selectedFacePoints[0].z)&&(obj.selectedFacePoints[0].x!=0)){
        let center:any;
        let ax = 0, ay = 0, az = 0;
        for(let a=0;a<myShape.length;a++){
          ax += myShape[a].x;
          ay += myShape[a].y;
          az += myShape[a].z;
        }  
        ax=ax/myShape.length;
        ay=ay/myShape.length;
        az=az/myShape.length;
        let myShapes: BABYLON.Vector3[] = [];
        for (let i = 0; i < myShape.length; i++) {
          let vector = new BABYLON.Vector3(myShape[i].x-ax, myShape[i].y - ay, myShape[i].z-az);
          myShapes.push(vector);
        }
        ...
        normalVec.normalize();
        let yradian;
        if(normalVec.z!=0)yradian=Math.atan(normalVec.x/normalVec.z)
        else yradian=Math.PI/2;  
        let right_direction=0;
        if((normalVec.x*normalVec.x*normalVec.z)>0){
          right_direction=1;
        }
        else {
          right_direction=-1;
        ...
        rotationMatrixz = BABYLON.Matrix.RotationAxis(BABYLON.Axis.Y, -yradian);
        ...
        const myPath = [
          new BABYLON.Vector3(0, 0, 0),
          new BABYLON.Vector3(0, 0, parseFloat(obj.heightExt))
        ];
        ...
        if(right_direction>0)
          ...
          if(normalVec.x>0){
              extrusion.position=new BABYLON.Vector3(
                      ax-parseFloat(obj.heightExt)*Math.sin(yradian),
                      ay,
                      az-parseFloat(obj.heightExt)*Math.cos(yradian)
              );
          }
          else{
              extrusion.position=new BABYLON.Vector3(
        ...
        let ax = 0, ay = 0, az = 0;
        if(obj.diameter!==undefined){
          for(let a=1;a<myShape.length-1;a++){
            ax += myShape[a].x;
            ay += myShape[a].y;
            az += myShape[a].z;
          }       
        ...
        if(obj.diameter!==undefined){
          myShapes = myShapes.filter((_, index) => index != 0);      
        ...
        // Normalize the normal vector
        normalVec.normalize();
        if(obj.diameter!==undefined)if(normalVec.y>0)swapArrayItems(myShapes,0,1);
        // let normalVec:BABYLON.Vector3 = new BABYLON.Vector3(0, 1,0);
        let rotationMatrixz: any;
        let anys: any;
        //Magnify plane
        if(obj.diameter===undefined){
        ...
        const myPath = [
          new BABYLON.Vector3(0, 0, 0),
          new BABYLON.Vector3(0, 0, right_direction*parseFloat(obj.heightExt))
        ];
        ...
        if(obj.diameter===undefined){
          if (anys == 1) {
            ...
            if(right_direction==1){console.log("cc");
              extrusion.rotation.x =obj.rotation.x;
              extrusion.rotation.y =obj.rotation.y;
              extrusion.rotation.z =obj.rotation.z;
              
              var endpivot=new BABYLON.Vector3(0,0,0);
              var S=new BABYLON.Vector3(0,0,0)
              var E=new BABYLON.Vector3(0,obj.selectedFaceScaling.y,0)
              var center=new BABYLON.Vector3(obj.selectedFaceScaling.x,obj.selectedFaceScaling.y/2,obj.selectedFaceScaling.z/2)
              var abs=new BABYLON.Vector3(obj.selectedFaceScaling.x/2,obj.selectedFaceScaling.y/2,obj.selectedFaceScaling.z/2);
              S=S.subtract(abs);
              E=E.subtract(abs);
              center=center.subtract(abs);
              var rotationMatrix = BABYLON.Matrix.RotationYawPitchRoll(obj.rotation.y, obj.rotation.x, obj.rotation.z);
              S = BABYLON.Vector3.TransformCoordinates(S, rotationMatrix);
              E = BABYLON.Vector3.TransformCoordinates(E, rotationMatrix);
              center = BABYLON.Vector3.TransformCoordinates(center, rotationMatrix);

              // Create lines representing the rotated line segment
              S=S.subtract(E);              
              extrusion.rotate(S,-Math.PI/2,BABYLON.Space.WORLD);
              extrusion.position=new BABYLON.Vector3(
          ...
          else if (anys == 2) {console.log("hh");
            // extrusion.rotation.x = Math.PI / 2;
            // extrusion.rotate(BABYLON.Axis.X,-Math.PI / 2,BABYLON.Space.WORLD);
            if(right_direction==1){
              ...
              extrusion.rotation.z =obj.rotation.z;
              var S=new BABYLON.Vector3(0,0,0)
              var E=new BABYLON.Vector3(obj.selectedFaceScaling.x,0,0)
              var center=new BABYLON.Vector3(obj.selectedFaceScaling.x/2,obj.selectedFaceScaling.y,obj.selectedFaceScaling.z/2)
              var abs=new BABYLON.Vector3(obj.selectedFaceScaling.x/2,obj.selectedFaceScaling.y/2,obj.selectedFaceScaling.z/2);
              S=S.subtract(abs);
              E=E.subtract(abs);
              center=center.subtract(abs);
              ...
              var S=new BABYLON.Vector3(0,0,0)
              var E=new BABYLON.Vector3(obj.selectedFaceScaling.x,0,0)
              var center=new BABYLON.Vector3(obj.selectedFaceScaling.x/2,0,obj.selectedFaceScaling.z/2)
              var abs=new BABYLON.Vector3(obj.selectedFaceScaling.x/2,obj.selectedFaceScaling.y/2,obj.selectedFaceScaling.z/2);
              S=S.subtract(abs);
              E=E.subtract(abs);
              center=center.subtract(abs);
            ...
            if(right_direction==1){
              console.log("nn");          
              // extrusion.position = new BABYLON.Vector3(obj.position.x,obj.position.y,obj.position.z+obj.selectedFaceScaling.z/2);
              extrusion.rotation.y =obj.rotation.y;
              extrusion.rotation.x =obj.rotation.x;
              extrusion.rotation.z =obj.rotation.z;
              extrusion.position=new BABYLON.Vector3(
              ...
              let l=obj.selectedFaceScaling.z/2;
              let y=l*Math.sin(obj.rotation.x);
              let x=-l*Math.cos(obj.rotation.x)*Math.sin(obj.rotation.y);
              let z=l*Math.cos(obj.rotation.x)*Math.cos(obj.rotation.y);
              extrusion.position=new BABYLON.Vector3(
                          extrusion.position.x-x,
                          extrusion.position.y-y,
                          extrusion.position.z+z
              );
            }
            else {
              console.log("gg");
          ...
          if (anys == 1) {console.log("zz");
            extrusion.rotation.y = Math.PI / 2;
            extrusion.rotation.x = -Math.PI / 2;
            extrusion.position = new BABYLON.Vector3(myShape[0].x,0,0);
          }
          else if (anys == 2) {console.log("bb",right_direction);                    
            if(right_direction==1){
              extrusion.rotation.x = -Math.PI / 2;  
              extrusion.position = new BABYLON.Vector3(obj.position.x,myShape[0].y+obj.position.y,obj.position.z);
            }
            else {
              extrusion.rotation.x =-Math.PI / 2;  
              extrusion.position = new BABYLON.Vector3(obj.position.x,myShape[0].y+obj.position.y,obj.position.z);
            ...
            console.log("cc");
            extrusion.rotation.x = -Math.PI / 2;
            extrusion.position = new BABYLON.Vector3(0,0,myShape[0].z);
          }
        }
