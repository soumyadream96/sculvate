...
function MainScene({ getScene, setObjects, objects }: MainSceneProps) {
  ...
  const addShape = async (type: string, obj: any) => {
    ...
    if (type === "cube") {
      ...
      // myShapes=[new BABYLON.Vector3(0,0,-2),new BABYLON.Vector3(2,0,0),new BABYLON.Vector3(0,0,-2),new BABYLON.Vector3(-2,0,0)]
      ...
      if(obj.diameter===undefined){
        if (anys == 1) {console.log("aa");
          ...
          if(right_direction==1)extrusion.position = new BABYLON.Vector3(parseFloat(obj.offsetPosition.x)+parseFloat(obj.selectedFaceScaling.x)+obj.position.x, parseFloat(obj.offsetPosition.y)+obj.selectedFaceScaling.y/2+obj.position.y,parseFloat(obj.offsetPosition.z)+obj.selectedFaceScaling.z/2+obj.position.z);
          else extrusion.position = new BABYLON.Vector3(parseFloat(obj.offsetPosition.x), parseFloat(obj.offsetPosition.y)+obj.selectedFaceScaling.y/2,parseFloat(obj.offsetPosition.z)+obj.selectedFaceScaling.z/2+obj.position.z);
        }
        else if (anys == 2) {console.log("bb");
          extrusion.rotation.x = Math.PI / 2;
          if(right_direction==1)extrusion.position = new BABYLON.Vector3(parseFloat(obj.offsetPosition.x)+obj.selectedFaceScaling.x/2, parseFloat(obj.offsetPosition.y)+parseFloat(obj.selectedFaceScaling.y)+parseFloat(obj.height)+obj.position.y, parseFloat(obj.offsetPosition.z)+obj.selectedFaceScaling.z/2+obj.position.z);
          else extrusion.position = new BABYLON.Vector3(parseFloat(obj.offsetPosition.x)+obj.selectedFaceScaling.x/2, parseFloat(obj.offsetPosition.y)-parseFloat(obj.height)+obj.position.y, parseFloat(obj.offsetPosition.z)+obj.selectedFaceScaling.z/2+obj.position.z);
        }
        else {
          console.log("cc");
          if(right_direction==1)extrusion.position = new BABYLON.Vector3(obj.position.x+parseFloat(obj.position.x-parseFloat(obj.offsetPosition.x)+parseFloat(obj.selectedFaceScaling.x)+obj.offsetPosition.x)+obj.selectedFaceScaling.x/2, parseFloat(obj.offsetPosition.y)+obj.selectedFaceScaling.y/2+obj.position.y, parseFloat(obj.offsetPosition.z)+parseFloat(obj.selectedFaceScaling.z)+obj.position.z);
          else extrusion.position = new BABYLON.Vector3(obj.position.x+parseFloat(obj.offsetPosition.x)+obj.selectedFaceScaling.x/2, parseFloat(obj.offsetPosition.y)+obj.selectedFaceScaling.y/2+obj.position.y, parseFloat(obj.offsetPosition.z)+obj.position.z);
        }
      }
      else{
        if (anys == 1) {console.log("aa");
          ...
          extrusion.rotation.x = -Math.PI / 2;          
          if(right_direction==1)extrusion.position = new BABYLON.Vector3(obj.position.x,myShape[0].y+obj.position.y,obj.position.z);
          else extrusion.position = new BABYLON.Vector3(obj.position.x,myShape[0].y+obj.position.y,obj.position.z);
        }
        else {
          console.log("cc");
          extrusion.rotation.x = -Math.PI / 2;
